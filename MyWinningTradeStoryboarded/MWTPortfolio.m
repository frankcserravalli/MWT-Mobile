...
//    [self displayDictionary:portfolio];
//    
...
//    _stocksArray = [[NSArray alloc] initWithArray:[self convertIntoArrayADictOf:_stocks]];
    ...
    {
//        NSDictionary *stockAtIndex = [_stocksArray objectAtIndex:i];
        MWTStock *stockAtIndex = [_stocksArray objectAtIndex:i];
//        NSString *symbol = [stockAtIndex objectForKey:@"symbol"];
        [sortedSymbolsArray addObject:stockAtIndex.symbol];
    }
...
- (id) initWith:(NSDictionary *)dictionary
{
    ...
    if (self)
    {
        ...
        _stocksArray = [self compileStocksFromDictionary:[dictionary objectForKey:@"stocks"]];
        _stockSymbols = [[dictionary objectForKey:@"stocks"] allKeys];
...
- (NSArray *) compileStocksFromDictionary:(NSDictionary *)dictionary
{
    NSMutableArray *stocks = [NSMutableArray array];
    
    for (NSString *key in dictionary)
    {
        NSDictionary *stockDict = [dictionary objectForKey:key];
        MWTStock *stock = [[MWTStock alloc] initWith:stockDict];
        stock.symbol = key;
